...
namespace TechSol.StudentManagementSystem.DAL
{
    ...
    public static class BaseRepository
    {
        ...
        internal static string sqlConnectionString = AppSettings.ConnectionString; //EncryptorDecryptorEngine.DecryptString(AppSettings.ConnectionString); //use for encrypted ConnectionString in appsettings.json
        ...
        public static async Task<T> Get<T>(object id, IDbTransaction transaction = null, int? commandTimeout = null, bool includeActive = true)
        {
            ...
            var task = Task.Run(() =>
            {
                ...
                //using (connection = new SqlConnection("Data Source = testvldb1.database.windows.net; initial catalog = CrossCheckDB; user id = sa_ss; password = 9009.westl00p"))
                //{
                //    if (connection.State != ConnectionState.Open)
                //    {
                //        connection.Open();
        ...
        public static async Task<IEnumerable<T>> GetList<T>(object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null, bool includeActive = true)
        {
            var currenttype = typeof(T);
            var name = GetTableName(currenttype);
        ...
        public static async Task<IEnumerable<T>> GetListUsingSP<T>(string procedureName, object paramters)
        {
            var task = Task.Run(() =>
            {
                IDbConnection connection = null;
                using (connection = new SqlConnection(sqlConnectionString))
                {
                    ...
                    foreach (var item in paramters as Dictionary<string, string>)
                    {
                        if (item.Key.Equals("orderby") && item.Value.Contains("_"))
                            queryParameters.Add(item.Key, item.Value.Replace("_", string.Empty));
                        else
                            queryParameters.Add(item.Key, item.Value);
                    }
                    var result = connection.Query<T>(procedureName, queryParameters, commandType: CommandType.StoredProcedure);
                    connection.Close();
                    return result;
                }
            });
        ...
        public static void InsertBulk<TEntity>(IEnumerable<TEntity> entitiesToInsert, int? commandTimeout = null)
        {
            TEntity entityToInsert = entitiesToInsert.FirstOrDefault();
            ...
            var task = Task.Run(() =>
            {
                ...
                using (connection = new SqlConnection(sqlConnectionString))
                {
                    ...
                    {
                        bulkCopy.BulkCopyTimeout = commandTimeout == null ? 0 : Convert.ToInt32(commandTimeout);
                        bulkCopy.BatchSize = 1000;
                        bulkCopy.DestinationTableName = name;
                        bulkCopy.EnableStreaming = true;

                        using (var dataReader = entitiesToInsert.GetDataReader())
                        {
        ...
        public static async Task<int> UpdateBulk<TEntity>(IEnumerable<TEntity> entitiesToInsert, string procedureName, string tvpName, int? commandTimeout = null)
        {
            DataTable dt = new DataTable();
            TEntity entityToInsert = entitiesToInsert.FirstOrDefault();
            PropertyInfo[] properties = entityToInsert.GetType().GetProperties();
            foreach (var property in properties)
            {
                Type t = property.PropertyType;
                Type underLyingType = t == typeof(Nullable) || t.Name.Contains("Nullable") ? Nullable.GetUnderlyingType(t) : t;

                string columnName = _columnNameResolver.ResolveColumnName(property);
                dt.Columns.Add(columnName, underLyingType);
            }
            int count = 1;
            foreach (var entity in entitiesToInsert)
            {
                ...
                foreach (var property in properties)
                {
                    try
                    {
                        row[_columnNameResolver.ResolveColumnName(property)] = property.GetValue(entity, null);
                    }
                    catch
                    {
                        row[_columnNameResolver.ResolveColumnName(property)] = DBNull.Value;
                    }
            ...
            var task = Task.Run(() =>
            {
                ...
                using (connection = new SqlConnection(sqlConnectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(procedureName, connection))
                    {
                        ...
                        SqlParameter tvpParam = command.Parameters.AddWithValue(tvpName, dt);
                        tvpParam.SqlDbType = SqlDbType.Structured;
                        var result = command.ExecuteNonQuery();
                        //var tvp = string.Concat("@", tvpName);
                        //var values = new { tvp = dt };
                        //var result = connection.Query(procedureName, values, commandType: CommandType.StoredProcedure);
                        //connection.Close();
                        return result;
                    }
                }
        ...
        public static async Task<int> SoftDelete<T>(object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var masterSb = new StringBuilder();
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_UpdateWhere{whereConditions?.GetType()?.FullName}", sb =>
            {
                var currenttype = typeof(T);
                var name = GetTableName(currenttype);

                var whereprops = GetAllProperties(whereConditions).ToArray();
                sb.AppendFormat("Update {0} Set Active = 0 ", name);
                if (whereprops.Any())
                {
                    sb.Append(" where ");
                    BuildWhere<T>(sb, whereprops);
                ...
                if (Debugger.IsAttached)
                    Trace.WriteLine(String.Format("UpdateList<{0}> {1}", currenttype, sb));
            });
        ...
        public static async Task<int> RecordCount<T>(Dictionary<string, object> conditions, string[] parameters, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (Debugger.IsAttached)
                Trace.WriteLine(String.Format("RecordCount<{0}>: {1}", currenttype, query));
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>Returns a count of records.</returns>
        public static async Task<T> CustomQuery<T>(string query, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);

            if (Debugger.IsAttached)
                Trace.WriteLine(String.Format("CustomQuery<{0}>: {1}", currenttype, query));
            ...
            var task = Task.Run(() =>
            {
                ...
                using (connection = new SqlConnection(sqlConnectionString))
                {
                    ...
                    var result = connection.Query<T>(query, null, transaction, true, commandTimeout).FirstOrDefault();
                    connection.Close();
                    return result;
                }
            });
        ...
        public static async Task<object> ScalarQuery(string query, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            //var currenttype = typeof(T);
            ...
            var task = Task.Run(() =>
            {
                ...
                using (connection = new SqlConnection(sqlConnectionString))
                {
                    ...
                    var result = connection.ExecuteScalar(query, null, transaction);
                    connection.Close();
                    return result;
                }
            });
        ...
        //Added by Waqar Ahmed
        public static async Task<T> Get<T>(string condition, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var sb = new StringBuilder();
            ...
            var task = Task.Run(() =>
            {
                ...
                using (connection = new SqlConnection(sqlConnectionString))
                {
                    ...
                    var result = connection.Query<T>(sb.ToString(), null, transaction, true, commandTimeout).FirstOrDefault();
                    connection.Close();
                    return result;
                }
            });
        ...
        /// <returns>Returns a count of records.</returns>
        /// 
        /*public static async Task<IEnumerable<T>> CustomQueryList<T>(string query, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
        ...
        public static async Task<IEnumerable<T>> CustomQueryList<T>(string condition, string order = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var sb = new StringBuilder();
            ...
            var tableName = GetTableName(currenttype);
            //var a = GetScaffoldableProperties<T>().ToArray();

            //string output = Regex.Replace(columns, "([a-z])_?([A-Z])", "$1 $2");
        ...
        public static async Task<IEnumerable<T>> CustomQueryList<T>(string query, object param, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            ...
            var task = Task.Run(() =>
            {
                ...
                using (connection = new SqlConnection(sqlConnectionString))
                {
                    ...
                    var result = connection.Query<T>(query, param, transaction, true, commandTimeout);
                    connection.Close();
                    return result;
                }
            });
        ...
        /// </summary>
        /// <param name="resolver">The resolver to use when 
        /// ing the format of a table name</param>
        ...
        private static string BuildWhere<TEntity>(Dictionary<string, object> conditions, string[] parameters)
        {
            ...
            for (var x = 0; x < sourceProperties.Count(); x++)
            {
                ...
                if (columnAttribute != null)
                {
                    ...
                    isColumnNumericDic.Add((columnAttribute as System.ComponentModel.DataAnnotations.Schema.ColumnAttribute).Name, t == null ? sourceProperties[x].PropertyType : t);
                }
            ...
            if (conditions.Keys.FirstOrDefault(x => x == "@keyfilter") != null)
            {
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    if (dicItemType != null && Common.IsNumericType(dicItemType))
                    {
                        if (double.TryParse(keyFilterValue, out double doubleResult) || int.TryParse(keyFilterValue, out int intResult))
                            where = where.Replace("/filterWhere/", string.Format("OR {1} like '%' + '{0}' + '%'/filterWhere/ ", keyFilterValue, parameters[i]));
                    }
                    else if (dicItemType != null && Common.IsDateType(dicItemType))
                    {
